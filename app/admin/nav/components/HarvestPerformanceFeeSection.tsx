    ...
    <div className="rounded-lg border border-neutral-200 bg-white p-4 dark:border-neutral-700 dark:bg-neutral-900">
      <div className="mb-4 flex items-center gap-2">
        ...
        <span className="rounded bg-neutral-100 px-2 py-0.5 text-xs text-neutral-500 dark:bg-neutral-800 dark:text-neutral-400">
          VaultManager.harvestPerformanceFee()
        </span>
      </div>

      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5 text-sm">
        <div>
          ...
          <p className="mt-0.5 font-semibold tabular-nums text-neutral-900 dark:text-white">
            {formatTokenAmount(data.performanceFeeRate, 16, 2)}%
          </p>
        </div>
        <div>
      ...
      {watermarkUninitialized ? (
        <p className="mb-4 text-xs text-blue-600 dark:text-blue-400">
          High watermark is uninitialized — the first harvest will bootstrap it to the current PPS and accrue zero shares.
        </p>
      ) : ppsBelowWatermark ? (
      ...
      <p className="mb-4 text-xs text-neutral-500 dark:text-neutral-400">
        Calling{' '}
        <code className="rounded bg-neutral-100 px-1 dark:bg-neutral-800">harvestPerformanceFee()</code>{' '}
        mints performance-fee shares against gains above the watermark and raises the watermark to the current PPS. The Safe must hold{' '}
