    ...
    queryFn: async () => {
      ...
      const enriched = decodedResults.map(({ tx, dataDecoded }) => {
        ...
        return {
          ...
          dataDecoded,
          summary: summarizeDecodedData(dataDecoded, tx.to, tx.value ?? '0', vaultAssetMap, assetMetadata ?? {}, chain),
          fulfillPrecheck,
...
async function enrichSafeTx(
  ...
  return {
    ...
    dataDecoded,
    summary: summarizeDecodedData(dataDecoded, tx.to, tx.value ?? '0', vaultAssetMap, assetMetadata, chainId),
    isExecuted: tx.isExecuted ?? false,
    isSuccessful: tx.isSuccessful ?? null,
    executionDate: tx.executionDate ?? null,
    transactionHash: tx.transactionHash ?? null,
